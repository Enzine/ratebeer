...
describe "BeermappingApi" do
  it "When HTTP GET returns one entry, it is parsed and returned" do
    ...
    <<-END_OF_STRING
      ...
      <id>12411</id><name>Gallows Bird</name><status>Brewery</status>
  ...
  it "When HTTP GET returns no entries, an empty array is returned" do
    ...
    canned_answer =
    <<-END_OF_STRING
    <?xml version='1.0' encoding='utf-8' ?><bmp_locations><location><id></id><name></name><status></status>
    <reviewlink></reviewlink><proxylink></proxylink><blogmap></blogmap><street></street><city></city>
    <state></state><zip></zip><country></country><phone></phone><overall></overall><imagecount></imagecount>
    </location></bmp_locations>
    END_OF_STRING

    stub_request(:get, /.*veikkola/).to_return(body: canned_answer, headers: { 'Content-Type' => "text/xml" })

    places = BeermappingApi.places_in("veikkola")

    expect(places).to be_empty
  end

  it "When HTTP GET returns many entries, an array of them is returned" do
